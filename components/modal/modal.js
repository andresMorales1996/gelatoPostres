...
function footerFormulario(producto) {
  ...
  boton.addEventListener("click", (event) => {
    event.preventDefault();
    AgregarProductoCarrito(producto.id, producto.nombre, producto.imagen);
    quitarAlerta();
  });
...
function mostrarPrecioAPagar() {
  ...
  const selecionarPrecio = document
    .getElementById("precios-productos")
    .value.split("x")[0];
  const $btn_agregar_carrito = document.getElementById(
  ...
  // Mostrar símbolo de peso cuando se selecciona una opción en la lista
  mostrarPrecio.classList.add("simbolo-peso");
  mostrarPrecio.textContent = parseFloat(
    selecionarPrecio * parseInt(p_cantidad.textContent)
  ).toFixed(3);
}
...
function quitarAlerta() {
  ...
  setTimeout(() => {
    alerta.remove();
    console.log("Se quitó la alerta");
  }, 5000);
}
...
function AgregarProductoCarrito(id, nombre, imagen) {
  ...
  const boton_ir_al_carrito = document.createElement("button");
  boton_ir_al_carrito.textContent = "Ir al carrito";
  boton_ir_al_carrito.addEventListener("click", function () {
    window.location.href = "../../pages/carrito.html";
  });
  create_alerta.appendChild(boton_ir_al_carrito);
  ...
  const informacionProducto = document
    .getElementById("precios-productos")
    .value.split("x");
  ...
  const cantidad = parseInt(
    document.getElementById("cant-producto").textContent
  );
